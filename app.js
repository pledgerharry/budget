...
const DEFAULT_BALANCES = {
  'Monzo':191.58,'Lloyds current':780.00,'Lloyds credit card':-9079.17
};
...
async function loadAll(){
  ...
  try{
    ...
    if(data.budgets&&Object.keys(data.budgets).length>0){
      ...
      // Sync any new default categories that might be missing or stale in the Sheet
      let needsUpdate=false;
      Object.entries(DEFAULT_BUDGETS).forEach(([cat,val])=>{
        if(state.budgets[cat]===undefined||(cat==='Holidays'&&state.budgets[cat]===0&&val>0)){
          state.budgets[cat]=val;
          needsUpdate=true;
        }
      });
      if(needsUpdate) await apiCall({action:'initBudgets',data:state.budgets});
    } else {
...
const FIXED_PAYMENTS = [
  ...
  {day:28, label:'Salary',            amt:3018.01, acc:'Lloyds current',     income:true},
...
function renderInsights(){
  ...
  if(upcoming.length>0){
    ...
    if(incoming.length>0){
      incoming.forEach(p=>{
        ...
        insights.push({
          type:'positive',icon:'💸',
          text:'Salary of '+fmt(p.amt)+' lands '+when+'. Once it hits, aim to immediately transfer at least £1,800 to the credit card before it gets spent on other things.'
        });
      });
    }
  }
  ...
  if(holidayBudget>0){
    const saved=holidaySpent; // treating Holidays spend as savings transfers
    const totalTarget=1200;
    // Estimate total saved across all months by checking payments
    const allHolidayTx=state.transactions.filter(t=>t.category==='Holidays');
    const totalSaved=allHolidayTx.reduce((s,t)=>s+t.amount,0);
    ...
    if(insights.length<5){
      insights.push({type: totalSaved>=totalTarget?'positive':'info', icon:'✈️',
        text: totalSaved>=totalTarget
          ...
          : 'Holiday fund: '+fmt(totalSaved)+' saved of '+fmt(totalTarget)+' goal ('+pctToGoal+'%). Saving '+fmt(holidayBudget)+'/month — on track to hit your target in around '+monthsLeft+' months. Log holiday savings transfers under the Holidays category.'
      });
    }
  }
