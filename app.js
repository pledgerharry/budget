...
// Pay period: 28th to 27th. Period "2026-05" = 28 Apr to 27 May.
function currentYM(){
  const now=new Date();
  const day=now.getDate(),y=now.getFullYear(),m=now.getMonth()+1;
  if(day>=28){
    return m===12?padYM(y+1,1):padYM(y,m+1);
  } else {
    return padYM(y,m);
  }
}

function inPeriod(dateStr,ym){
  if(!dateStr||!ym) return false;
  const d=String(dateStr).split('T')[0];
  const py=parseInt(ym.slice(0,4)),pm=parseInt(ym.slice(5,7));
  const prevM=pm===1?12:pm-1;
  const prevY=pm===1?py-1:py;
  const start=prevY+'-'+String(prevM).padStart(2,'0')+'-28';
  const end=ym+'-27';
...
function fmtYM(ym){
  ...
  const prevM=pm===1?12:pm-1;
  const prevY=pm===1?py-1:py;
  const s=new Date(prevY,prevM-1,28);
  const e=new Date(py,pm-1,27);
...
function renderGlance(){
  ...
  // Disposable budget — always sum from individual category budgets
  const dispBudget=DISPOSABLE_CATS.reduce((s,c)=>s+(parseFloat(state.budgets[c])||DEFAULT_BUDGETS[c]||0),0);
  const dailyBudget=Math.round((dispBudget/28)*100)/100;

  // Disposable spend so far this period
  ...
  // Available cash: Monzo + Lloyds current minus £50 buffer
  const monzo=state.balances['Monzo']||0;
  const lloyds=state.balances['Lloyds current']||0;
  const BUFFER=50;
  const availableCash=Math.max(0,(monzo+lloyds)-BUFFER);
  const cashPerDay=daysToPayday>0?Math.round((availableCash/daysToPayday)*100)/100:0;

  // Surplus vs budget pace
  const idealSpend=dailyBudget*daysToPayday;
  const surplus=dispRem-idealSpend;

  const pct=Math.min(100,Math.round((dispSpent/dispBudget)*100));
  ...
  if(barLabel) barLabel.textContent='Disposable: '+pct+'% of '+fmt(dispBudget)+' budget used — '+fmtYM(ym);

  const el=document.getElementById('glance');
  if(el) el.innerHTML=
    '<div class="row"><span class="row-label">Disposable budget</span><span class="row-val">'+fmt(dispBudget)+'</span></div>'
    +'<div class="row"><span class="row-label">Spent so far</span><span class="row-val red">'+fmt(dispSpent)+'</span></div>'
    +'<div class="row"><span class="row-label">Remaining (budget)</span><span class="row-val '+(dispRem>=0?'green':'red')+'">'+fmt(dispRem)+'</span></div>'
    +'<div class="row"><span class="row-label">Days to payday</span><span class="row-val">'+daysToPayday+' days</span></div>'
    +'<div class="row"><span class="row-label">Budget pace</span><span class="row-val">'+fmt(dailyBudget)+'/day</span></div>'
    +'<div style="border-top:1px solid #222;margin:0">'
    +'<div class="row"><span class="row-label">Liquid (excl. £50 buffer)</span><span class="row-val '+(availableCash>100?'green':availableCash>0?'amber':'red')+'">'+fmt(availableCash)+'</span></div>'
    +'<div class="row"><span class="row-label">Cash per day to payday</span><span class="row-val '+(cashPerDay>=dailyBudget?'green':cashPerDay>0?'amber':'red')+'">'+fmt(cashPerDay)+'/day</span></div>'
    +'</div>';
  ...
  if(surplusEl){
    if(surplus>20){
      surplusEl.innerHTML=
        ...
        +'<div style="padding:10px 14px;font-size:12px;color:#888">You are ahead of your '+fmt(dailyBudget)+'/day pace. Consider moving '+fmt(surplus)+' to the credit card today.</div>';
      ...
      surplusEl.innerHTML=
        ...
        +'<div style="padding:10px 14px;font-size:12px;color:#888">Try to keep daily spend under '+fmt(daysToPayday>0?dispRem/daysToPayday:0)+' for the rest of the month.</div>';
      surplusEl.style.display='';
    } else {
...
function renderDebt(){
  const cc=state.balances['Lloyds credit card']||0;
  const owed=Math.abs(cc),start_bal=Math.abs(CC_START);
  const paid=Math.max(0,start_bal-owed),pct=Math.min(100,Math.round((paid/start_bal)*100));
  ...
  if(el) el.innerHTML=
    ...
    +'<div class="row"><span class="row-label">Started at</span><span class="row-val">'+fmt(start_bal)+'</span></div>'
    +'<div class="row"><span class="row-label">Paid off</span><span class="row-val green">'+fmt(paid)+'</span></div>'
  ...
  // Payoff scenarios — use live balance and show actual month/year
  const mr=APR/12;
  const proj=document.getElementById('debtProjection');
  if(!proj) return;

  const now=new Date();
  // Current pay period: if day >= 28 we're in the next period
  const periodStart=now.getDate()>=28
    ? new Date(now.getFullYear(), now.getMonth(), 28)
    : new Date(now.getFullYear(), now.getMonth()-1, 28);

  function clearDate(monthsFromNow){
    const d=new Date(periodStart.getFullYear(), periodStart.getMonth()+monthsFromNow, 28);
    return d.toLocaleString('en-GB',{month:'long',year:'numeric'});
  }

  proj.innerHTML=[400,600,800,1000,1200,1500,1800].map(pmt=>{
    if(pmt<=owed*mr){
      return'<div class="scenario-row"><span class="row-label" style="font-family:Space Mono,monospace;font-size:12px">£'+pmt+'/mo</span><span class="badge b-bad">never clears</span></div>';
    }
    ...
    const col=months<=6?'#44ff88':months<=12?'#ffaa00':'#ff4444';
    const cls=months<=6?'b-ok':months<=12?'b-warn':'b-bad';
    return'<div class="scenario-row">'
      ...
      +'<span class="badge '+cls+'" style="color:'+col+'">'+clearDate(months)+'</span>'
